 ...
 * @brief File with method definitions for the (EC) group 1 on a cryptographic
 * pairing
 *
 * This file defines the type and methods for manipulating elements form the
 * first group (additive notation) of a cryptographic pairing, including
 * generators, serialization, addition, multiplication...
 * 
...
/**
 * Encapsulated declaration of type G1, which represents elements from G1 of the
 * pairing
 */
...
/**
 * @brief Generates a copy of the group generator. Can be safely modified. Has
...
/**
 * @brief Generates a copy of group identity (addition operation). Can be safely
 * modified. Has to be freed after usage
 * 
 * @return 
 */
G1* g1Identity();
...
/**
 * @brief Represent G1 element as byte array. Array is assumed to be big enough
 * for copy
 * 
...
/**
 * @brief Generates a G1 element from bytes (previously serialized). Has to be
 * freed after usage
 * 
 * @param bytes Byte array of serialized element (assumed to be the correct
 * length, i.e. g1ByteSyze())
 */
...
/**
 * @brief Multiplication [b] lt[0], with lt being a lookup table for G1 element
 * g with lt[i]=g^(2^(i+1)). Result must be freed.
 * 
...
/**
 * @brief Multiplication res= [b] lt[0], with lt being a lookup table for G1
 * element g with lt[i]=g^(2^(i+1)).
 * 
 * @param res Will contain the result of the computation, should have been
 * allocated before
...
/**
 * @brief Destroy G1 element, freeing memory (from malloc etc.) as
 * needed. Depending on internal structure might be equivalent to free(e)
 * 
 * @param e 
 */
void g1Free(G1* e);
