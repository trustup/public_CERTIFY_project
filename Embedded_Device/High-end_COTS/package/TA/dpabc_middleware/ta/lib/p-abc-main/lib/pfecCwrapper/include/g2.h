 ...
 * @brief File with method definitions for the (EC) group 2 on a cryptographic
 * pairing
 *
 * This file defines the type and methods for manipulating elements form the
 * second group (additive notation) of a cryptographic pairing, including
...
/**
 * Encapsulated declaration of type G2, which represents elements from G2 of the
 * pairing
 */
...
/**
 * @brief Represent G2 element as byte array. Array is assumed to be big enough
 * for copy
 * 
...
/**
 * @brief Generates a G2 element from bytes (previously serialized). Has to be
 * freed after usage
 * 
 * @param bytes Byte array of serialized element (assumed to be the correct
 * length, i.e. g2ByteSyze())
 */
...
/**
 * @brief Destroy G2 element, freeing memory (from malloc etc.) as
