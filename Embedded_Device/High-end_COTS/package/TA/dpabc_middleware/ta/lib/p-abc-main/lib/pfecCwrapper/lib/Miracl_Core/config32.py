...
my_compiler = os.environ.get('AARCH32_COMPILER', "gcc")
generated_files = []
...
class miracl_compile:
    def compile_file(optim, file):
        ...
        if optim != 0:
            flags = " -std=c99 -O%d -c %s -fPIC" % (optim, file)
        else:
            flags = " -std=c99 -c %s -fPIC" % (file)
    ...
    def compile_binary(optim, file, lib, bin):
        ...
        if sys.platform.startswith("win"):
            bin += ".exe"
        if optim != 0:
            flags = " -std=c99 -O%d %s %s -o %s -fPIC" % (optim, file, lib, bin)
        else:
            flags = " -std=c99 %s %s -o %s -fPIC" % (file, lib, bin)
        os.system(my_compiler + flags)
        print(". [DONE]")
