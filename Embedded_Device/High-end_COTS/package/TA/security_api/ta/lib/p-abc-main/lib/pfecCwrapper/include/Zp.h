/**
 * @file Zp.h
 * @brief File with method definitions for integers modulo p, with p prime (used
 * in schemes that take advantage of EC crypto)
 *
 * @details This file defines the type and methods for manipulating integers
 * modulo p, with p prime which will be decided at compile time depending on the
 * instantiation of this wrapper library (depends on the needs of the
 * pariring-friendly-EC based scheme)
 * 
...
/**
 * Encapsulated declaration of type Zp, which represents integers modulo p, p
 * prime The internal management of modular expressions is up to implementation,
 * but it must conceptually work as if everything is always being computed
 * within the field Zp
 */
...
/**
 * @brief Generates a Zp element from bytes (previously serialized). Has to be
 * freed after usage
 * 
 * @param bytes Byte array of serialized element (assumed to be the correct
 * length, i.e. zpByteSyze())
 */
Zp * zpFromBytes(const char *bytes);

/**
 * @brief Construct a new Zp element, hashing from bytes. Has to be freed after
 * usage
 * 
...
/**
 * @brief Return the number of bits (ignoring left-zeros, i.e. position of most
 * significant bit counting from the least significant bit as 1) of a
 * 
...
/**
 * @brief Destroy Zp element, freeing memory (from malloc etc.) as
