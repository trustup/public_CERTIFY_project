...
/**
 * Encapsulated declaration of type ranGen, which represents random generator
 * structure (can be initialized with a seed, used to generate random bytes...)
 */
typedef struct ranGen ranGen;

//TODO One or multiple functions to recover information (depending on how we
//model it) about the concrete implementation

/**
 * @brief Initialize a ranGen (random generator) with seed. Must be freed after
 * usage.
 * 
...
/**
 * @brief Destroy Random Generator, freeing memory (from malloc etc.) as
 * needed. Depending on internal structure might be equivalent to free(rg)
 * 
