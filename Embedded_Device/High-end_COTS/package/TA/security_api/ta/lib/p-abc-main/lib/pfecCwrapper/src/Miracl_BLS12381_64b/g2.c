...
static void hash_to_field_BLS12381_G2(int hash,int hlen,FP2_BLS12381 *u,octet *DST,octet *M, int ctr)
{
    ...
    for (i=0;i<ctr;i++)
    {
        ...
        BIG_384_58_dmod(w1,dx,q);
        for (j=0;j<L;j++)
            fd[j]=OKM.val[(2*i+1)*L+j];
        BIG_384_58_dfromBytesLen(dx,fd,L);
        BIG_384_58_dmod(w2,dx,q);
...
int g2ByteSize(){
    return 4*MODBYTES_384_58+1; //0x04|x|y with x,y in FP2 serialized as a|b from FP
}
...
void g2ToBytes(char *res, const G2 *a){
	if (g2IsIdentity(a)) {
		...
		octet o={0,4*MODBYTES_384_58+1,res};
...
G2 * g2FromBytes(const char *bytes){
	if (bytes[0] == 0x6c) {
		...
		G2 *r=malloc(sizeof(G2));
		r->p=malloc(sizeof(ECP2_BLS12381));
		octet o={0,4*MODBYTES_384_58+1,bytes};
