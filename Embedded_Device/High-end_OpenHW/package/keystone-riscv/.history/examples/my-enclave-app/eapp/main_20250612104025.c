...
void mi_strncpy(char* dest, const char* src, size_t n) {
  size_t i = 0;
  // Copiar mientras haya caracteres en src y no hayamos llegado al límite n
  while (i < n && src[i] != '\0') {
      dest[i] = src[i];
      i++;
  }

  // Rellenar con '\0' si no se llenó todo n
  while (i < n) {
      dest[i] = '\0';
      i++;
  }
