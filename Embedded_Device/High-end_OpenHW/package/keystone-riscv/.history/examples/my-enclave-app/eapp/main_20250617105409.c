...
EAPP_ENTRY eapp_entry(){
  ...
  switch (cmd) {
    ...
    case CMD_SIGN:
      ...
      // void *data_to_sign = malloc(msg_sign.size);
      // copy_from_shared(data_to_sign, msg_sign.offset, msg_sign.size);
      
      // // Get the key to use for signing
      // struct edge_data msg_key;
      // ocall_wait_for_message(&msg_key);
      // void *signing_key = malloc(msg_key.size);
      // copy_from_shared(signing_key, msg_key.offset, msg_key.size);
      
      // // Generate signature
      // uint8_t signature[SIGNATURE_SIZE];
      // sign_data(data_to_sign, msg_sign.size, signing_key, msg_key.size, signature);
      
      // // Send signature back
      // struct edge_data msg_sig_out;
      // msg_sig_out.offset = 0;
      // msg_sig_out.size = SIGNATURE_SIZE;
      // copy_to_shared(signature, msg_sig_out.offset, msg_sig_out.size);
      // ocall_send_message(&msg_sig_out);
      ...
      // print_hex("CMAC-AES", signature + MD5_DIGEST_SIZE, 16);
