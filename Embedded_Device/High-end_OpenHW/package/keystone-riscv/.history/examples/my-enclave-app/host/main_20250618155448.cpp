...
struct edge_data wait_for_message() {
  ...
  if (actual_test_case == CMD_INSTALL_PSK) {
    ...
    // Leer el MUD URL cifrado del archivo
    int fd = open(KDK_PATH, O_RDONLY);
    if (fd < 0) {
      perror("[HOST] No se pudo abrir el archivo AK para enviar");
      return message; // mensaje nulo
    }

    uint8_t* buffer = (uint8_t*)malloc(PSK_SIZE);
    if (!buffer) {
      perror("[HOST] malloc falló para enviar el kdk cifrado");
      close(fd);
      return message;
    }

    ssize_t bytes_read = read(fd, buffer, PSK_SIZE);
    close(fd);

    if (bytes_read != PSK_SIZE) {
      fprintf(stderr, "[HOST] Error: esperado 64 bytes para el kdk, leído %ld\n", bytes_read);
      free(buffer);
      return message;
    }

    printf("[HOST] Enviando kdk desde archivo\n");

    message.offset = (uintptr_t)buffer;
    message.size = PSK_SIZE;



  }else if (actual_test_case == CMD_SIGN){
    // Preparar datos para firma: key_id, algoritmo y datos a firmar
    const char* key_id = "identifier1"; // Puede ser "PSK" o "MSK" o cualquier key_id válido
    const char* algorithm = CMAC_AES; // Por defecto usar CMAC_AES
    ...
    // Calcular tamaño total: [key_id_length][key_id][algorithm_length][algorithm][data_length][data]
    ...
    // Escribir longitud del algoritmo
    memcpy(ptr, &algorithm_len, sizeof(size_t));
    ptr += sizeof(size_t);
    
    // Escribir algoritmo
    memcpy(ptr, algorithm, algorithm_len);
    ptr += algorithm_len;
...
void print_write_key_materials(void* buffer) {
  ...
  // --- Guardar claves ---
  const char* ak_path = AK_PATH;
  const char* kdk_path = KDK_PATH;
