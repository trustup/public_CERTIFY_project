...
void print_write_key_materials(void* buffer) {
  ...
  const char* ak_path = (keys->type == 1) ? "/etc/myapp/key_material/ak_msk.bin" : AK_PSK_PATH;
  const char* kdk_path = (keys->type == 1) ? "/etc/myapp/key_material/kdk_msk.bin" : KDK_PSK_PATH;
