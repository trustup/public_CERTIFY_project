...
struct edge_data wait_for_message() {
  ...
  if (actual_test_case == CMD_INSTALL_PSK) {
    ...
    const char* key_path = (derive_session == SESSION_PSK_KDK) ? KDK_PSK_PATH : KDK_MSK_PATH;
    int iterations = (key_path == KDK_PSK_PATH) ? 1 : 9;
    for (int i = 0; i < iterations; ++i) {
      ...
      printf("[HOST] Enviando datos para derivación - Sesión: %d (%s), Iteración: %d\n", 
             derive_session, 
             (derive_session == SESSION_PSK_KDK) ? "PSK" : "MSK",
             i+1);

      message.offset = (uintptr_t)buffer;
      message.size = total_size;
      // Si solo quieres enviar una vez en el caso de PSK, puedes hacer break aquí
      // if (derive_session == SESSION_PSK_KDK) break;
    }
...
void print_write_key_materials(void* buffer) {
  ...
  if (stat(KEY_MATERIAL_DIR, &st) == -1) {
    ...
    if (mkdir(KEY_MATERIAL_DIR, 0700) != 0) {
      perror("[HOST] No se pudo crear el directorio para key_material");
      edge_call->return_data.call_status = CALL_STATUS_BAD_PTR;
      return;
    }
  }
