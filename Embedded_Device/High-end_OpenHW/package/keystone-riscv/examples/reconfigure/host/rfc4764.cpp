...
static uint32_t verify_server(char * s_cmac, char * id_s, size_t id_s_sz, char * rand_p, char * psk_id) {
	...
	char * generated_mac_s = (char *)malloc(AES128_BLOCK_SIZE);
	if (!generated_mac_s) {
		free(conc);
		return OUT_OF_MEMORY;
	}
...
static uint8_t * marshal_header(eap_psk_header header) {
	...
	uint8_t * header_bytes = (uint8_t *)malloc(HEADER_SIZE);
...
static uint32_t send_second_msg(int sockfd, char * rand_s, char * rand_p, char * cmac, char * id_p, size_t id_p_sz, char * s_cmac, char * psk_id, char ** pchannel, size_t * pchannel_sz) {
	...
	second_message * message = (second_message *)malloc(second_message_sz);
	...
	uint8_t * marshal_second_message = (uint8_t *)malloc(second_message_sz - sizeof(eap_psk_header) + HEADER_SIZE);
    ...
    // Log: mostrar todo lo que se está enviando en el segundo mensaje
    // printf("[DEBUG] marshal_second_message size: %zu\n", second_message_sz - sizeof(eap_psk_header) + HEADER_SIZE);
    // printf("[DEBUG] marshal_second_message (primeros 64 bytes en hex): ");
    // for (size_t i = 0; i < ((second_message_sz - sizeof(eap_psk_header) + HEADER_SIZE) < 64 ? (second_message_sz - sizeof(eap_psk_header) + HEADER_SIZE) : 64); ++i) {
    //     printf("%02x ", ((unsigned char*)marshal_second_message)[i]);
    // }
    // printf("\n");
    // printf("[DEBUG] marshal_second_message (primeros 64 bytes como string segura): %.*s\n",
    //     (int)(((second_message_sz - sizeof(eap_psk_header) + HEADER_SIZE) < 64 ? (second_message_sz - sizeof(eap_psk_header) + HEADER_SIZE) : 64)),
    //     (const char*)marshal_second_message);
	...
	// Print mac_s después de la firma
	// printf("[DEBUG] response->mac_s (hex): ");
	// for (int i = 0; i < 16; ++i) printf("%02x ", ((unsigned char*)response->mac_s)[i]);
	// printf("\n");
	// printf("send_second_message");
	...
	*pchannel = (char *)malloc(*pchannel_sz);
...
static uint32_t send_fourth_msg(int sockfd, char * rand_s, char * pchannel, size_t pchannel_sz) {
	...
	fourth_message * message = (fourth_message *)malloc(fourth_message_sz);
	...
	uint8_t * marshaled_header = marshal_header(message->header);
	uint8_t * marshal_fourth_message = (uint8_t *)malloc(fourth_message_sz - sizeof(eap_psk_header) + HEADER_SIZE);
...
static uint32_t initiate_authentication(int sockfd, char ** rand_s, char ** id_s, size_t * id_s_sz) {
	...
	printf("[HOST] Contents of first 16 bytes of received buffer (first EAP message):\n");
	DBG_print_block(rec_buff);
	
	message = (first_message*) rec_buff;
	// Rest is client

	// printf("initiate authentication");
	if ((res = check_header(message->header, 0))) {
		printf("EAP-PSK header format error\n");
		goto exit;
	}

	printf("[HOST] EAP-PSK first message received\n");
	// printf("Code: %x\n", message->header.code);
	// printf("Id: %x\n", message->header.id);
	...
	*rand_s = (char *)malloc(AES128_BLOCK_SIZE);
	if (!*rand_s) {
		res = 1;
		goto exit;
	}
...
static uint32_t send_hello(char * url, size_t url_size, int * sockfd, uint8_t key_code) {
	...
	// printf("[DEBUG] send_hello: INICIO url=%s, url_size=%zu, key_code=0x%02x\n", url, url_size, key_code);
	uint32_t res;
	char rec_buff[DEFAULT_BUFFER_SZ] = { 0 };
	size_t rec_size;

	char * hello_message = (char *)malloc(4 + url_size);
	// printf("[DEBUG] send_hello: hello_message malloc %p\n", (void*)hello_message);
	...
	// printf("[DEBUG] send_hello: hello_message construido, primeros bytes: %02x %02x %02x %02x\n", hello_message[0], hello_message[1], hello_message[2], hello_message[3]);
	// printf("[DEBUG] send_hello: url copiado: %.*s\n", (int)url_size, hello_message+4);

	// res = send_bytes("155.54.95.211", 33333, (uint8_t*)hello_message, 4 + url_size, sockfd);
	// printf("[DEBUG] send_hello: llamando a send_bytes\n");
	res = send_bytes("155.54.95.175", 33333, (uint8_t*)hello_message, 4 + url_size, sockfd);
	// res = send_bytes("192.168.1.43", 33333, (uint8_t*)hello_message, 4 + url_size, sockfd);
	// printf("[DEBUG] send_hello: send_bytes res=%u, sockfd=%d\n", res, *sockfd);
	if (res != SUCCESS) {
		// printf("[DEBUG] send_hello: Error sending hello message\n");
		return res;
	}
	
	rec_size = 2; // OK or KO
	// printf("[DEBUG] send_hello: llamando a receive_bytes_socket\n");
	res = receive_bytes_socket(*sockfd, rec_buff, &rec_size);
	// printf("[DEBUG] send_hello: receive_bytes_socket res=%u, rec_size=%zu, rec_buff='%s'\n", res, rec_size, rec_buff);
	if (res != SUCCESS || !rec_size) {
		// printf("[DEBUG] send_hello: Error receiving hello response\n");
		close(*sockfd);
	...
	if (strcmp(rec_buff, "OK")) {
		// printf("[DEBUG] send_hello: respuesta no es OK ('%s')\n", rec_buff);
		close(*sockfd);
		return INVALID_SERVER_RESPONSE; 
	}

	// printf("[DEBUG] send_hello: FIN (OK)\n");
	return SUCCESS;
}
...
static uint32_t handle_success(int sockfd) {
	...
	uint32_t res;
	char rec_buff[DEFAULT_BUFFER_SZ];
	// En C para ok son 2 byte, para c++ son 3 ya que es o,k,\0
	char snd_buff[3] = "OK";
...
static uint32_t handle_id_request(int * sockfd, int * self_socket) {
	...
	uint16_t response_length = 5 + P_ID_SZ; // mac string size (17)
...
uint32_t join_request(char * url, char * psk_id, char * msk_id) {
	// printf("[DEBUG] join_request: INICIO url=%s, psk_id=%s, msk_id=%s\n", url, psk_id, msk_id);
	uint32_t res;
	...
	// printf("[DEBUG] join_request: Determinando bootstrapp_code\n");
	...
	// printf("[DEBUG] join_request: bootstrapp_code=%02x\n", bootstrapp_code);
	printf("[HOST] Sending hello message for stablize the conection\n");
	res = send_hello(url, strlen(url), &serv_sock, bootstrapp_code);
	printf("[HOST] Hello message sent succesfully and received for the server\n");
	// printf("[DEBUG] join_request: send_hello res=%u, serv_sock=%d\n", res, serv_sock);

	if (res) {
		// printf("[DEBUG] join_request: Failed to send hello message\n");
		return res;
	}

	printf("[HOST] Handling ID request from server\n");
	res = handle_id_request(&serv_sock, &self_sock);
	printf("[HOST] ID request handled succesfully\n");
	// printf("[DEBUG] join_request: handle_id_request res=%u, self_sock=%d\n", res, self_sock);

	if (res) {
		// printf("[DEBUG] join_request: Failed handle id request\n");
		goto close_sock_exit;
	}

	printf("[HOST] Initiating authentication with server\n");
	res = initiate_authentication(serv_sock, &rand_s, &id_s, &id_s_sz);
	printf("[HOST] Authentication initiated succesfully\n");
	// printf("[DEBUG] join_request: initiate_authentication res=%u\n", res);
	// DBG_print_block(rand_s);

	if (res) {
		// printf("[DEBUG] join_request: Failed to initiate authentication with bootstrapping server (0x%08x)\n", res);
		goto close_sock_exit;
	}

	res = csp_generateRandom((unsigned char *)rand_p, AES128_BLOCK_SIZE);
	// printf("[DEBUG] join_request: csp_generateRandom res=%u\n", res);
	if (res != SUCCESS) {
		// printf("[DEBUG] join_request: Failed to generate random buffer\n");	
		goto close_sock_exit;
	}

	cmac = (char *)malloc(AES128_BLOCK_SIZE);
	if (!cmac) {
		// printf("[DEBUG] join_request: OUT_OF_MEMORY al reservar cmac\n");
		res = OUT_OF_MEMORY;
		goto close_sock_exit;
	}

	printf("[HOST] Generating authentication\n");
	res = generate_auth(cmac, id_p, id_p_sz, id_s, id_s_sz, rand_s, rand_p, psk_id);
	printf("[HOST] Authentication generated succesfully\n");
	// printf("[DEBUG] join_request: generate_auth res=%u\n", res);
	if (res) {
		// printf("[DEBUG] join_request: Failed to generate authentication: (0x%08x)\n", res);
		goto close_sock_exit;
	}

	// printf("[DEBUG] join_request: CMAC generado\n");
	DBG_print_block(cmac);

	printf("[HOST] Sending second message to server\n");
	res = send_second_msg(serv_sock, rand_s, rand_p, cmac, id_p, id_p_sz, s_cmac, psk_id, &pchannel, &pchannel_sz); 
	printf("[HOST] Second message sent succesfully and response received from server\n");
	// printf("[DEBUG] join_request: send_second_msg res=%u\n", res);

	if (res) {
		// printf("[DEBUG] join_request: Failed key exchange with bootstrapping server: (0x%08x)\n", res);
		goto close_sock_exit;
	}

	// printf("[DEBUG] join_request: Server CMAC\n");
	DBG_print_block(s_cmac);
	
	printf("[HOST] Verifying server authentication\n");
	res = verify_server(s_cmac, id_s, id_s_sz, rand_p, psk_id);
	printf("[HOST] Server authentication verified succesfully\n");
	// printf("[DEBUG] join_request: verify_server res=%u\n", res);
	if (res) {
		// printf("[DEBUG] join_request: Failed to generate authentication: (0x%08x)\n", res);
		goto close_sock_exit;
	}
	else {
		// printf("[DEBUG] join_request: Server authenticated successfully\n");
	}

	printf("[HOST] Deriving key\n");
	res = csp_deriveKey(psk_id, msk_id, (unsigned char *)rand_p, NULL, 0, AES128);
	printf("[HOST] Key derived succesfully\n");
	// printf("[DEBUG] join_request: csp_deriveKey res=%u\n", res);
	if (res != SUCCESS) {
		// printf("[DEBUG] join_request: Error during key derivation\n");
		return res;
	}
	else {
		// printf("[DEBUG] join_request: Successfully derived MSK\n");
	}

	printf("[HOST] Sending fourth message to server\n");
	res = send_fourth_msg(serv_sock, rand_s, pchannel, pchannel_sz);
	printf("[HOST] Fourth message sent succesfully\n");
	// printf("[DEBUG] join_request: send_fourth_msg res=%u\n", res);
	if (res) {
		// printf("[DEBUG] join_request: Failed to send fourth EAP message (0x%08x)\n", res);
		goto close_sock_exit;
	}
	else {
		// printf("[DEBUG] join_request: Successfully sent fourth EAP message\n");
	}
	// Arrancar servidor de reconfiguración siempre que psk_id != PSK_KEY_ID ANTES de handle_success
	if (strcmp(psk_id, PSK_KEY_ID)) {
		run_reconfigure_server();
	}
	res = handle_success(serv_sock);
	// printf("[DEBUG] join_request: handle_success res=%u\n", res);

close_sock_exit:
	// printf("[DEBUG] join_request: Cerrando sockets serv_sock=%d, self_sock=%d\n", serv_sock, self_sock);
	close(serv_sock);
	close(self_sock);
	// printf("[DEBUG] join_request: FIN res=%u\n", res);
	return res;
}
