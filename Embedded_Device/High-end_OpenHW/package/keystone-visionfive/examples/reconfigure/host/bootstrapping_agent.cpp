...
int32_t bsa_sendJoinRequest(char * psk_id, char * msk_id) {
	...
	printf("[HOST] Inizializing join request using %s for derive %s\n", psk_id, msk_id);
	...
	printf("[HOST] Getting MudFileURL");
	...
	if (mud_url_end) {
	*mud_url_end = 0;
		// printf("[DEBUG] bsa_sendJoinRequest: mud_url_end found, mud_url=%s\n", mUDuRLValue);
	} else {
		// printf("[DEBUG] bsa_sendJoinRequest: mud_url_end not found , mud_url=%s\n", mUDuRLValue);
	}
	// printf("[DEBUG] bsa_sendJoinRequest: llamando a join_request\n");
	printf("[HOST] Inicializing join_request\n");
