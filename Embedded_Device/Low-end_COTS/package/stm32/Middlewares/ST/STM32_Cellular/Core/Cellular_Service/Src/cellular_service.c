...
/**
  ...
  * @note   This function has to be called periodically by the client in bare mode.
  * @param  event_callback client callback to call when an event occurred
  */
...
int32_t CS_sim_generic_access(CS_sim_generic_access_t *sim_generic_access)
{
  ...
  PRINT_API("CS_sim_generic_access")
      ...
      && (sim_generic_access->rsp_str_size >= CONFIG_MODEM_MIN_SIM_GENERIC_ACCESS_RSP_SIZE))
  {
    ...
    */
    ...
    {
      at_status_t err;
      err = AT_sendcmd(_Adapter_Handle, (at_msg_t) SID_CS_SIM_GENERIC_ACCESS, &cmd_buf[0], &rsp_buf[0]);
      if (err == ATSTATUS_OK)
      {
