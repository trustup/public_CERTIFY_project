...
CK_RV LoadTokenObjects()
{
  ...
  */
  ...
  if (rv == CKR_OK)
  {
	...
	*/
	rv = getDataObjectLengthAndValue((CK_BYTE)0x01, &dataObjectLen, &dataObjectValue[0]); /* Certificate */
	if (rv == CKR_OK)
	{
		/*
...
CK_RV getDataObjectLengthAndValue(CK_BYTE targetFile, uint16_t *dataObjectLen, CK_BYTE_PTR pDataObjectValue)
{
	...
	CK_RV lRes;
	CK_ULONG ulPath = 0;
	CK_BYTE pTmpBuf[150]; // *********** ATTENZIONE: PER MISRA QUI CI VUOLE UNA COSTANTE AL POSTO DI 150
	(void)memset(pTmpBuf, 0, 150);

	pTargetDF[ulPath] = 0x2F00U | targetFile;
	ulPath++;
	lRes = PathSelectWORD(pTargetDF, ulPath); /* Select file that contains the Data Object*/
	if (lRes == CKR_OK)
	{
		lRes = ReadBinary(0, 150, pTmpBuf);
		if (lRes == CKR_OK)
		{
