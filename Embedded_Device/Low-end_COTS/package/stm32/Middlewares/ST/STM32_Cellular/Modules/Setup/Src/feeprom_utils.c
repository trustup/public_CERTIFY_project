...
static uint32_t feeprom_utils_flash_write(uint8_t *data_addr, uint8_t *flash_addr, uint32_t Length,
...
/**
  * @brief  Write "Length" bytes in feeprom at "flash_addr
                                          ...
                                          uint32_t *byteswritten)
{
  ...
  *byteswritten = 0U;
  if (tmp_Length > FLASH_PAGE_SIZE)
  {
    tmp_Length = FLASH_PAGE_SIZE;
  }

  while (*byteswritten < tmp_Length)
  {
        ...
        *addr        = setup_config;
        if (setup_config->header.appli_version != appli_version)
        {
          ...
          *page_number = (uint32_t)FLASH_LAST_PAGE_NUMBER - i;
          ...
          PRINT_SETUP("WARNING : \"%s\"  FEEPROM Config Bad format : config is erased\r\n", label)
          ...
          PRINT_SETUP("Press any key to continue...\r\n")
          (void)menu_utils_get_char_timeout(&car, 30000U);
        }
        else
        {
          /* matching bank found */
