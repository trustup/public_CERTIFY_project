...
/* Setup version for HTTP */
#define HTTP_CLIENT_SETUP_VERSION      (uint16_t)2

/* Loca port used when creating the socket */
...
/* Error counter implementation : nb of consecutive errors before to start NFM feature or Modem reset */
#define HTTP_CLIENT_ERROR_SHORT_LIMIT_MAX        5U

/* Gandi server */
#define DNS_SERVER_NAME        ((uint8_t*)"gandi.net")   /* name used for DNS resolution */
#define DNS_SERVER_IP          ((uint32_t)0x41B946D9U)   /* 217.70.185.65 */
#define DNS_SERVER_PORT        ((uint16_t)443U)          /* remote port   */

/* Cloud server */
#define CLOUD_SERVER_NAME      ((uint8_t*)"liveobjects.orange-business.com") /* name used for DNS resolution */
#define CLOUD_SERVER_IP        ((uint32_t)0xD02A2754U)                       /*  84.39.42.208 */
#define CLOUD_SERVER_PORT      ((uint16_t)8883U)                             /* remote port   */

/* Internal test server */
#define ST_UNKNOWN_NAME        ((uint8_t*)"stunknown.st")
#define ST_UNKNOWN_IP          ((uint32_t)0x9B36379AU)    /* 154.55.54.155 */
#define ST_UNKNOWN_PORT        ((uint16_t)1234U)          /* remote port   */
...
/* Name that appears in the menu configuration */
#define HTTPCLIENT_LABEL  ((uint8_t*)"Grovestreams")

/* umber of default HTTP client client parameters */
#define HTTPCLIENT_DEFAULT_PARAM_NB 21U

/* Begin - Values used to generate PUT request : data send to the remote server */
#define HTTPCLIENT_PUT_BATLEVEL_CHANNEL_ID         0U  /* Put channel ID value for Battery level          */
#define HTTPCLIENT_PUT_SIGLEVEL_CHANNEL_ID         1U  /* Put channel ID value for Signal level           */
#define HTTPCLIENT_PUT_ICCID_CHANNEL_ID            2U  /* Put channel ID value for ICCID value            */
#define HTTPCLIENT_PUT_IMSI_CHANNEL_ID             3U  /* Put channel ID value for IMSI value             */
#define HTTPCLIENT_PUT_HUM_CHANNEL_ID              4U  /* Put channel ID value for Humidity value         */
#define HTTPCLIENT_PUT_TEMP_CHANNEL_ID             5U  /* Put channel ID value for Temperature value      */
#define HTTPCLIENT_PUT_PRESS_CHANNEL_ID            6U  /* Put channel ID value for Pressure value         */
#define HTTPCLIENT_PUT_ACCX_CHANNEL_ID             7U  /* Put channel ID value for Accelerometer X value  */
#define HTTPCLIENT_PUT_ACCY_CHANNEL_ID             8U  /* Put channel ID value for Accelerometer Y value  */
#define HTTPCLIENT_PUT_ACCZ_CHANNEL_ID             9U  /* Put channel ID value for Accelerometer Y value  */
#define HTTPCLIENT_PUT_GYRX_CHANNEL_ID            10U  /* Put channel ID value for Gyroscope X value      */
#define HTTPCLIENT_PUT_GYRY_CHANNEL_ID            11U  /* Put channel ID value for Gyroscope Y value      */
#define HTTPCLIENT_PUT_GYRZ_CHANNEL_ID            12U  /* Put channel ID value for Gyroscope Z value      */
#define HTTPCLIENT_PUT_MAGX_CHANNEL_ID            13U  /* Put channel ID value for Magnetometer X value   */
#define HTTPCLIENT_PUT_MAGY_CHANNEL_ID            14U  /* Put channel ID value for Magnetometer Y value   */
#define HTTPCLIENT_PUT_MAGZ_CHANNEL_ID            15U  /* Put channel ID value for Magnetometer Z value   */

#if (USE_DC_GENERIC == 1)
#define HTTPCLIENT_PUT_GEN_BOOL1_CHANNEL_ID       16U  /* Put channel ID value for a Boolean 1 value      */
#define HTTPCLIENT_PUT_GEN_BOOL2_CHANNEL_ID       17U  /* Put channel ID value for a Boolean 2 value      */
#define HTTPCLIENT_PUT_GEN_BYTE1_CHANNEL_ID       18U  /* Put channel ID value for a Byte 1 value         */
#define HTTPCLIENT_PUT_GEN_BYTE2_CHANNEL_ID       19U  /* Put channel ID value for a Byte 2 value         */
#define HTTPCLIENT_PUT_GEN_LONG1_CHANNEL_ID       20U  /* Put channel ID value for a Long 1 value         */
#define HTTPCLIENT_PUT_GEN_LONG2_CHANNEL_ID       21U  /* Put channel ID value for a Long 2 value         */
#define HTTPCLIENT_PUT_GEN_FLOAT1_CHANNEL_ID      22U  /* Put channel ID value for a Float 1 value        */
#define HTTPCLIENT_PUT_GEN_FLOAT2_CHANNEL_ID      23U  /* Put channel ID value for a Float 2 value        */
#define PUT_CHANNEL_ID_MAX              ((uint8_t)24U) /* Put channel ID max value                        */
#else
#define PUT_CHANNEL_ID_MAX              ((uint8_t)16U) /* Put channel ID max value                        */
#endif /* USE_DC_GENERIC == 1 */
/* End - Values used to generate PUT request : data send to the remote server */

/* Begin - Values used to generate Get request : data receive from the remote server */
#define HTTPCLIENT_GET_LEDLIGHT_CHANNEL_ID        0U   /* Get channel ID value for Led light value       */

#if (USE_DC_GENERIC == 1)
#define HTTPCLIENT_GET_GEN_BOOL1_CHANNEL_ID       1U   /* Get channel ID value for a Boolean 1 value     */
#define HTTPCLIENT_GET_GEN_BOOL2_CHANNEL_ID       2U   /* Get channel ID value for a Boolean 2 value     */
#define HTTPCLIENT_GET_GEN_BYTE1_CHANNEL_ID       3U   /* Get channel ID value for a Byte 1 value        */
#define HTTPCLIENT_GET_GEN_BYTE2_CHANNEL_ID       4U   /* Get channel ID value for a Byte 2 value        */
#define HTTPCLIENT_GET_GEN_LONG1_CHANNEL_ID       5U   /* Get channel ID value for a Long 1 value        */
#define HTTPCLIENT_GET_GEN_LONG2_CHANNEL_ID       6U   /* Get channel ID value for a Long 2 value        */
#define HTTPCLIENT_GET_GEN_FLOAT1_CHANNEL_ID      7U   /* Get channel ID value for a Float 1 value       */
#define HTTPCLIENT_GET_GEN_FLOAT2_CHANNEL_ID      8U   /* Get channel ID value for a Float 2 value       */
#define GET_CHANNEL_ID_MAX              ((uint8_t)9U)  /* Get channel ID max value                       */
#else
#define GET_CHANNEL_ID_MAX              ((uint8_t)2U)  /* Get channel ID max value                       */
#endif /* USE_DC_GENERIC == 1 */
/* End - Values used to generate Get request : data receive from the remote server */

#define HTTPCLIENT_SEND_PERIOD              1000U  /* period between two send in ms. */
#define CLIENT_MESSAGE_SIZE                 1500U  /* buffer size max in bytes */

#define KEY_GET_INPUT_SIZE                    50U  /* Size max to input Key Get */
#define KEY_PUT_INPUT_SIZE     KEY_GET_INPUT_SIZE  /* Size max to input Key Put */
#define CHANNEL_ID_SIZE_MAX                   15U  /* Size max to input a channel ID */
#define COMPONENT_ID_STRING_SIZE_MAX          15U  /* Size max to input a component ID */
#define SELECT_INPUT_STRING_SIZE               5U

#define PUT_REQUEST_PERIOD_SIZE_MAX            5U  /* Size max to enter Put request period */
#define GET_REQUEST_PERIOD_SIZE_MAX            5U  /* Size max to enter Gett request period */
...
typedef struct
{
  uint32_t count;    /* count number of http done     */
  uint32_t cnt_ok;   /* count number of connection ok */
  uint32_t cnt_ko;   /* count number of connection ko */
  uint32_t snd_ok;   /* count number of send       ok */
  uint32_t snd_ko;   /* count number of send       ko */
  uint32_t rcv_ok;   /* count number of receive    ok */
  uint32_t rcv_ko;   /* count number of receive    ko */
  uint32_t cls_ok;   /* count number of close      ok */
  uint32_t cls_ko;   /* count number of close      ok */
} httpclient_stat_t;
...
/* Formatted http GET request buffer */
static com_char_t formatted_http_request[400U];
/* Formatted http PUT request buffer */
...
#if (USE_RTC == 1)
/* Set or not Date/Time => a specific request will be send to the echo remote server */
static bool http_client_set_date_time; /* false: date time has not to be requested,
                                          true:  date time has to be requested,
                                          default value : see http_client_init() */
#endif /* (USE_RTC == 1) */

static uint32_t http_client_put_period; /* Period for Put request in ms */
static uint32_t http_client_get_period; /* Period for Gett request in ms */
...
static bool http_client_network_is_on;
...
/* Callback */
static void http_client_notif_cb(dc_com_event_id_t dc_event_id, const void *p_private_gui_data);
...
static bool http_client_get_nfmc_parameters(uint8_t index,
                                            uint32_t *value);
static void http_client_socket_thread(void *p_argument);
...
static uint32_t http_client_sensor_put_list_config(void);
static uint32_t http_client_device_get_list_config(void);
...
static void http_client_cmd_help(void)
{
  /* Display information about http cmd and its supported parameters */
...
static cmd_status_t http_client_cmd(uint8_t *cmd_line_p)
{
  ...
  cmd_status = CMD_OK;
  uint32_t total_ko; /* calcul number of http ko */

  cmd_p = (uint8_t *)strtok((CRC_CHAR_t *)cmd_line_p, " \t");

  if (cmd_p != NULL)
  {
    /* Is it a command httpclient ? */
        ...
        == 0)
    {
      ...
      {
        /* cmd 'http help': display help */
        http_client_cmd_help();
      }
      else if (memcmp((CRC_CHAR_t *)argv_p[0], "on", crs_strlen(argv_p[0])) == 0)
      {
        /* cmd 'http on': start http */
        http_client_process_flag = true;
        PRINT_FORCE("\n\r<<< HTTPCLIENT ACTIVE >>>")
      }
      else if (memcmp((CRC_CHAR_t *)argv_p[0], "off", crs_strlen(argv_p[0])) == 0)
      {
        /* cmd 'http off': stop http */
        http_client_process_flag = false;
        PRINT_FORCE("\n\r<<< HTTPCLIENT NOT ACTIVE >>>")
      }
      else if (memcmp((CRC_CHAR_t *)argv_p[0], "valid", crs_strlen(argv_p[0])) == 0)
      {
        /* cmd 'http valid stat': use for automatic test to obtain the statistics */
        if (memcmp((CRC_CHAR_t *)argv_p[1], "stat", crs_strlen(argv_p[1])) == 0)
        {
          /* cmd 'http valid stat': use for automatic test to obtain the statistics */
      ...
      else if (memcmp((CRC_CHAR_t *)argv_p[0], "stat", crs_strlen(argv_p[0])) == 0)
      {
        /* cmd 'http stat' : display http statistics */
      ...
      else if (memcmp((CRC_CHAR_t *)argv_p[0], "resetstats", crs_strlen(argv_p[0])) == 0)
      {
        /* cmd 'http resetstats' : reset http statistics */
        (void)memset((void *)&httpclient_stat, 0, sizeof(httpclient_stat));

        PRINT_FORCE("Httpclient statistics reset")
      }
      else
      {
...
static void http_client_notif_cb(dc_com_event_id_t dc_event_id, const void *p_private_gui_data)
{
  ...
  if (dc_event_id == DC_CELLULAR_NIFMAN_INFO)
  {
    ...
    if (dc_nifman_info.rt_state == DC_SERVICE_ON)
    {
      ...
      http_client_network_is_on = true;
    }
    else if (dc_nifman_info.rt_state == DC_SERVICE_SHUTTING_DOWN)
    {
      ...
      http_client_network_is_on = false;
    }
    else
    {
      /* Network is down */
  ...
  /* Event DC_CELLULAR_INFO allow to follow modem reset */
...
#if (USE_BUTTONS == 1)
  /* Is a button pressed ? */
  /* Press button up same result
     than cmd 'http on' if http is inactive or 'http off' if http is active */
  else if (dc_event_id == DC_BOARD_BUTTONS_UP)
  {
...
/**
  ...
  * @param  str_result_len - length of returned string
  * @note   -
  * @retval uint32_t
  */
                                     ...
                                     com_ip_addr_t *hostip)
{
  ...
  if (res == 0U)
  {
           ...
           && (result == false))
    {
      ...
      {
        /* length are equal, compare the string */
    ...
    if (result == false)
    {
      uint32_t ip_string_len = crs_strlen(&ip_string[0]) + 1U;
...
{
  uint8_t i;
  dc_cellular_info_t      cellular_info; /* for Signal level, ICCID value */
  dc_sim_info_t           sim_info;      /* for IMSI value */
#if (USE_DC_EMUL == 1)
  dc_battery_info_t       battery_info;  /* for Battery level */
#endif /* USE_DC_EMUL == 1 */
#if ((USE_DC_MEMS == 1) || (USE_SIMU_MEMS == 1))
...
static void http_client_check_distantname(void)
{
      ...
      && (httpclient_distantip.addr == 0U))
  {
        ...
        == COM_SOCKETS_ERR_OK)
    {
      ...
      */
      {
        /* DNS resolution ok - save IP in httpclient_distantip */
...
static void http_client_create_socket(void)
{
  ...
  if (socket_state == SOCKET_INVALID)
  {
    ...
    if (socket_http_client >= 0)
    {
      ...
      if (com_bind(socket_http_client,
          ...
          == COM_SOCKETS_ERR_OK)
      {
        /* Bind is ok, call rcvtimeout and sdntimeout setsockopt */
  ...
  /* If socket is created, continue the process : connect to the remote server */
  if (socket_state == SOCKET_CREATED)
  {
    address.sin_family      = (uint8_t)COM_AF_INET;
    ...
    else
    {
      ...
      /* If unitary test is activated,
         internal server name is unknown by DNS: don't reset distant IP address */
...
/**
  ...
  * @note   -
  * @retval bool    - false : error occurred before to receive a full trame
  *                   true  : full trame received
  */
static bool http_client_receive_a_full_trame(com_char_t *buf_rsp)
{
  bool result;    /* no error occurred when reading data ? true */
  ...
  uint8_t *p_string;       /* string is available in the response or not */
  ...
  /* Clean-Up the response buffer
     Put '\0' to be sure that even if the string is not completely read string is ended
     Assume response length < CLIENT_MESSAGE_SIZE
   */
  (void)memset((void *)buf_rsp, (int32_t)'\0', CLIENT_MESSAGE_SIZE);

  do
  {
    /* Wait data from remote server */
    ...
    if (rsp_size > 0)
    {
       ...
       *  4) if Content Length != 0 data are available in the response
      ...
      if (p_string != NULL)
      {
        ...
        if (content_length > 0)
        {
          ...
          /* Data are expected - Check if all data are received */
                                ...
                                com_char_t *buf_rsp)
{
  ...
  if (httpclient_distantip.addr != (uint32_t)0U)
  {
    ...
    {
      /* Socket is in good state to continue send data to the remote server */
          ...
          > 0)
      {
        ...
        if (result == false)
        {
          /* Response ko, increase fault counters, request close socket */
          socket_closing = true;
          httpclient_stat.rcv_ko++;
        }
        else
        {
          ...
          if (http_client_set_date_time == true)
          {
            ...
            if (ret != TIMEDATE_STATUS_OK)
            {
              ...
              {
                /* Time date information maybe available */
                PRINT_INFO("Update date and time")
                if (timedate_set_from_http((uint8_t *)&buf_rsp[header_size]) != TIMEDATE_STATUS_OK)
                {
          ...
          || (socket_state == SOCKET_WAITING_RSP))
      {
        ...
        if (com_closesocket(socket_http_client) != COM_SOCKETS_ERR_OK)
        {
          /* close is ko, increase fault counter */
          socket_state = SOCKET_CLOSING;
...
static void http_client_configuration(void)
{
  ...
  if (ip_valid == false)
  {
    ...
    (void)memcpy(httpclient_distantname,
                 HTTPCLIENT_DEFAULT_DISTANTNAME,
                 crs_strlen(HTTPCLIENT_DEFAULT_DISTANTNAME) + 1U);
  }
...
/**
  ...
  * @note   Configure 'PUT' sensor list
  * @param  -
...
static uint32_t http_client_sensor_put_list_config(void)
{
  ...
  while (count < PUT_CHANNEL_ID_MAX)
  {
    PRINT_SETUP("\r\nConfig a sensor:\r\n")
    ...
    atoi_res--;
    if (atoi_res < (int32_t)PUT_CHANNEL_ID_MAX)
    {
      PRINT_SETUP("Enter channel ID of the sensor")

      menu_utils_get_string((uint8_t *)"Channel ID ",
                            put_channel_id_string_tab[(uint8_t)atoi_res],
                            CHANNEL_ID_SIZE_MAX);
      count++;
    }
    else
    {
      PRINT_SETUP("Bad channel ID (max %d)", PUT_CHANNEL_ID_MAX - 1U)
    }
  }
...
/**
  * @brief  Device list configuration
  * @note  Configure 'GET' Device list
  * @param  -
  * @retval uint32_t - number of sensor configured
  */
static uint32_t http_client_device_get_list_config(void)
{
  ...
  while (count < GET_CHANNEL_ID_MAX)
  {
    PRINT_SETUP("\r\nConfig a device:\r\n")
    PRINT_SETUP("Select device type")
    PRINT_SETUP("  0 : quit")
    menu_utils_get_string((uint8_t *)"Selection (0 to quit) ",
                          input_string,
                          SELECT_INPUT_STRING_SIZE);
    ...
    atoi_res--;
    if (atoi_res < (int32_t)GET_CHANNEL_ID_MAX)
    {
      PRINT_SETUP("Enter channel ID of the device")

      menu_utils_get_string((uint8_t *)"Channel ID ",
                            get_channel_id_string_tab[(uint8_t)atoi_res],
                            CHANNEL_ID_SIZE_MAX);
      count++;
    }
    else
    {
      PRINT_SETUP("Bad channel ID (max %d)", GET_CHANNEL_ID_MAX - 1U)
    }
  }
...
static uint32_t http_client_setup_handler(void)
{
  ...
  uint8_t input_period_string[HTTPCLIENT_MAX((PUT_REQUEST_PERIOD_SIZE_MAX + 1U), \
                                             (GET_REQUEST_PERIOD_SIZE_MAX + 1U))];
  uint8_t  i;
  ...
  */
  ...
  {
    http_client_put_period = (uint32_t)atoi_res;
    put_request_channel_count = http_client_sensor_put_list_config();
  ...
  {
    http_client_get_period = (uint32_t)atoi_res;
    get_request_channel_count = http_client_device_get_list_config();
...
static void http_client_setup_dump(void)
{
  ...
  if (http_client_put_period > 0U)
  {
    for (uint8_t i = 0U; i < PUT_CHANNEL_ID_MAX; i++)
    {
      if (put_channel_id_string_tab[i][0] != (uint8_t)'\0')
      {
        PRINT_FORCE("Channel: Type '%d' / ID '%s'", i + 1U, put_channel_id_string_tab[i]);
      }
    }
  }
  ...
  if (http_client_get_period > 0U)
  {
    for (uint8_t i = 0U; i < GET_CHANNEL_ID_MAX; i++)
    {
      if (get_channel_id_string_tab[i][0] != (uint8_t)'\0')
      {
        PRINT_FORCE("Channel: Type '%d' / ID '%s'", i + 1U, get_channel_id_string_tab[i]);
      }
    }
  }
}
...
static void http_client_socket_thread(void *p_argument)
{
  ...
  for (;;)
  {
    if (http_client_network_is_on == true)
    {
    ...
    else
    {
      ...
      {
        /* Nothing to do except to wait network is on */
    ...
    {
      local_ip_displayed = false;
      while (http_client_network_is_on == true)
      {
        if (http_client_process_flag == false)
        {
        ...
        else
        {
          ...
          {
            PRINT_DBG("Delay")
            (void)rtosalDelay(HTTPCLIENT_SEND_PERIOD);
          }
        }
      }
    }
...
{
  /* Put Channel ID Initialization */
  for (uint8_t i = 0U; i < PUT_CHANNEL_ID_MAX; i++)
  {
    put_channel_id_string_tab[i][0] = (uint8_t)('\0');
  }
  /* Get Channel ID Initialization */
  for (uint8_t i = 0U; i < GET_CHANNEL_ID_MAX; i++)
  {
    get_channel_id_string_tab[i][0] = (uint8_t)('\0');
  }
...
void httpclient_start(void)
{
  ...
  /* If HTTP is deactivated then another Appli is in charge to set date/time */
#if (USE_RTC == 1)
  ...
  httpClientTaskHandle = rtosalThreadNew((const rtosal_char_t *)"HttpCltThread", (os_pthread)http_client_socket_thread,
                                         HTTPCLIENT_THREAD_PRIO, USED_HTTPCLIENT_THREAD_STACK_SIZE, NULL);
  ...
  else
  {
    ...
    (void)stackAnalysis_addStackSizeByHandle(httpClientTaskHandle, USED_HTTPCLIENT_THREAD_STACK_SIZE);
