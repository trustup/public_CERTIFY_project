...
static int32_t mx_wifi_if_connect_ap(net_if_handle_t *pnetif)
{
  ...
  (void) sprintf(ap_cfg.ip.localip, "%d.%d.%d.%d", ADDR(pnetif->static_ipaddr.addr));
  (void) sprintf(ap_cfg.ip.netmask, "%d.%d.%d.%d", ADDR(pnetif->static_gateway.addr));
  (void) sprintf(ap_cfg.ip.gateway, "%d.%d.%d.%d", ADDR(pnetif->static_netmask.addr));
  (void) sprintf(ap_cfg.ip.dnserver, "%d.%d.%d.%d", ADDR(pnetif->static_dnserver.addr));
