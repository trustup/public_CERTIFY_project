...
#define PRINT_APP(format, args...) \
  TRACE_PRINT_FORCE(DBG_CHAN_CUSTOMCLIENT, DBL_LVL_P0, "" format, ## args) //DBG_CHAN_CUSTOMCLIENT
...
/**
  ...
  * @note   encryptKeyKeyID admitted values: "PSK", "MSK", "PSK_1", "PSK_2", "PSK_3", "PSK_4", "PSK_5", "PSK_6", "PSK_7", "PSK_8".
  * @param  encryptKeyKeyID    	 - Key ID of the encryption key
  * @param  dataToEncrypt        - Data to encrypt
  * @param  dataToEncryptLen     - Size of data (max len = 239 byte)
  * @param  EncryptedData        - The location that receives the cryptogram
  * @retval return 0 means OK
  */
int32_t csp_encrypt(char* encryptKeyID, unsigned char* dataToEncrypt, uint16_t dataToEncryptLen, unsigned char* EncryptedData)
{
  ...
  {
    {CKA_CLASS,    &secClass,   sizeof(secClass)},
    {CKA_LABEL,     encryptKeyID,   strlen(encryptKeyID)}
  ...
  rv = P11(C_EncryptInit)(hSession, &mechanism, hSecretKey);
  if (rv != CKR_OK)
  {
    PRINT_APP("C_EncryptInit, expected CKR_OK but was: 0x%08X \n", rv);
    goto exit;
  }

  rv = P11(C_Encrypt)(hSession, dataToEncrypt, dataToEncryptLen, EncryptedData,&enc_size);
  if (rv != CKR_OK)
  {
    PRINT_APP("C_Encrypt, expected CKR_OK but was: 0x%08X \n", rv);
  }
exit:
  return rv;
}
...
/**
  ...
  * @note   decryptKeyID admitted values: "PSK", "MSK", "PSK_1", "PSK_2", "PSK_3", "PSK_4", "PSK_5", "PSK_6", "PSK_7", "PSK_8".
  * @param  decryptKeyID    	- Key ID of the decription key
  * @param  dataToDecrypt       - Data to decrypt
  * @param  dataToDecryptLen    - Size of data (max len = 242 byte)
  * @param  PlainText        	- The location that receives the plain text
  * @retval return 0 means OK
  */
int32_t csp_decrypt(char* decryptKeyID, unsigned char* dataToDecrypt, uint16_t dataToDecryptLen, unsigned char* PlainText)
{
  ...
  {
    {CKA_CLASS,    &secClass,   sizeof(secClass)},
    {CKA_LABEL,     decryptKeyID,   strlen(decryptKeyID)}
  ...
  rv = P11(C_DecryptInit)(hSession, &mechanism, hSecretKey);
  if (rv != CKR_OK)
  {
    PRINT_APP("C_DecryptInit, expected CKR_OK but was: 0x%08X \n", rv);
    goto exit;
  }

  rv = P11(C_Decrypt)(hSession, dataToDecrypt, dataToDecryptLen, PlainText,&dec_size);
  if (rv != CKR_OK)
  {
    PRINT_APP("C_Decrypt, expected CKR_OK but was: 0x%08X \n", rv);
  }
exit:
  return rv;
}
...
int32_t csp_getCertificateValue(unsigned char* certificateValue, uint16_t *certificateValueLen)
{
	CK_OBJECT_CLASS crtClass = CKO_CERTIFICATE;
	uint32_t size=0;
	CK_BYTE certValue[1000];
	memset(certValue,0x00,1000);
