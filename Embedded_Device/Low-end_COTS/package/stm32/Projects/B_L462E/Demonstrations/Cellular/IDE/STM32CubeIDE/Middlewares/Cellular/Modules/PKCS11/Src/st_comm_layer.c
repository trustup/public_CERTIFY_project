...
{
	CK_ULONG retValue = CKR_OK;
	com_char_t buf_rsp[MAX_BUFFER_LENGTH_FOR_ENCDEC];
	int32_t ret;
	uint8_t charstr[5];
  	(void)memset(buf_rsp, 0x00, MAX_BUFFER_LENGTH_FOR_ENCDEC); /* Cleaning Response buffer */
	com_char_t data[3];
	(void)convertHexByteToAscii(pxSessionObj->targetKeyParameter, &data[0], &data[1]);
	/*Compute signature*/
	com_char_t pSendBuffer[MAX_BUFFER_LENGTH_FOR_ENCDEC];
	(void)memset(pSendBuffer, 0x00, MAX_BUFFER_LENGTH_FOR_ENCDEC);

	/* prepare input buffer - command  */
	pSendBuffer[0] = 0x42; /* CLA - B0 */
	pSendBuffer[1] = 0x30;

	pSendBuffer[2] = 0x46; /* INS - 0xF5*/
	pSendBuffer[3] = 0x35;

	pSendBuffer[4] = 0x30;/* P1    ‘01’ mode encipher    */
	pSendBuffer[5] = 0x31;

	pSendBuffer[6] = data[0]; /* P2  */
	pSendBuffer[7] = data[1];

	(void)convertHexByteToAscii(DataToEncryptLen, &data[0], &data[1]);
	pSendBuffer[8] = data[0]; /* Lc  */
	...
	/* Transmit to lower level */
	ret = com_icc_generic_access(h_icc, pSendBuffer, len, buf_rsp, MAX_BUFFER_LENGTH_FOR_ENCDEC);
                          ...
                          CK_BYTE *pData, CK_ULONG_PTR puDataLen)
{
	...
	pSendBuffer[4] = 0x30;/* P1    ‘02’ mode decipher    */
	pSendBuffer[5] = 0x32;

	pSendBuffer[6] = data[0]; /* P2  */
	pSendBuffer[7] = data[1];

	(void)convertHexByteToAscii(pEncryptedDataLen, &data[0], &data[1]);
	pSendBuffer[8] = data[0]; /* Lc  */
	pSendBuffer[9] = data[1];
...
CK_ULONG DeriveSecret(CK_HKDF_PARAMS pParams, int8_t * baseKeyLabel, char * derivedKeyLabel, CK_BYTE *pSecret, int32_t *pulSecret)
{
	...
	else if (strcmp((CCHAR *)baseKeyLabel, FIRST_DERIVED_KEY) == 0)
	{
		...
		pSendBuffer[5] = 0x30;
		if(strcmp((CCHAR *)derivedKeyLabel, ENC_DEC_KEY) == 0){
			pSendBuffer[6] = 0x30; /* P2 - 0x08*/
			pSendBuffer[7] = 0x38;
		}else{
