...
//				  }
	  ...
	  case CKO_DATA: /* Is a Data object */
	  {
		  for (iAttrib = 0; (iAttrib < ulCount) && (xResult == CKR_OK); iAttrib++)
		  {
			  ...
			  case CKA_VALUE:
				  xResult = getDataObjectLengthAndValue((CK_BYTE)0x01,&DataObjectFileContentLen, &DataObjectValue[0]);
				  if (xResult == CKR_OK)
				  {
                                       ...
                                       CK_OBJECT_HANDLE xKey)
{
  ...
  */
  ...
  if (xResult == CKR_OK)
  {
	  ...
	  if (strcmp((CCHAR *)p11Obj.xLabel, ENC_DEC_KEY) == 0)
	  {
		  pxSession->targetKeyParameter = (CK_BYTE)0x08;
	  }
	  else
	  {
		  xResult = CKR_KEY_HANDLE_INVALID;
...
/**
  ...
  * @param[in] ulDataLen                     Plaintext data max len = 239 byte.
...
CK_DECLARE_FUNCTION(CK_RV, C_Encrypt)(CK_SESSION_HANDLE xSession,
                                     ...
                                     CK_ULONG_PTR puEncryptedDataLen)
  {
    ...
    */
    ...
    if (xResult == CKR_OK)
    {
      if ((uDataLen == (uint32_t)0) || (NULL == pData) || (NULL == pEncryptedData) || (NULL == puEncryptedDataLen))
      {
        xResult = CKR_ARGUMENTS_BAD;
      }
    }
    ...
    if (xResult == CKR_OK)
    {
    	if (MAX_ENC_BUFFER_LENGTH < uDataLen)
    	{
    		xResult = CKR_DATA_LEN_RANGE;
    	}
    }
...
CK_DECLARE_FUNCTION(CK_RV, C_Decrypt)(CK_SESSION_HANDLE xSession,
                                     ...
                                     CK_ULONG_PTR uDataLen)
  {
    ...
    */
    ...
    if (xResult == CKR_OK)
    {
      if ((puEncryptedDataLen == (uint32_t)0) || (NULL == pEncryptedData) || (NULL == pData) || (NULL == uDataLen))
      {
        xResult = CKR_ARGUMENTS_BAD;
      }
    }
    ...
    if (xResult == CKR_OK)
    {
       	if (puEncryptedDataLen > MAX_DEC_BUFFER_LENGTH)
    	{
    		xResult = CKR_DATA_LEN_RANGE;
    	}
    }
    ...
    if (xResult == CKR_OK)
    {
    	xResult = decrypt(pxSessionObj, pEncryptedData,
    			(CK_BYTE)puEncryptedDataLen,
				pData,
				uDataLen);
    }
    /*
